...
listHistorics* lifoX (process * dummy,int nb){
  ...
  listHistorics* head=NULL;
    ...
    size_t arraySize = nb;
    qsort(dummy, arraySize, sizeof(process), compareStructsV9);
  ...
  addNewCommingToQueueLifo(&q,dummy,time,arraySize,&head);
   ...
   if(notEmpty(&q)){
     ...
     for(int i=time;i<time+current.te;i++){
        addNewCommingToQueueLifo(&q,dummy,i,arraySize,&head);
